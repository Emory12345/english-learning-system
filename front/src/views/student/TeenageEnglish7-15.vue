<template>
  <div class="teenage-english-7-15">
    <h2>7-15岁 基础英语</h2>
    <p class="subtitle">系统学习英语，打好基础，稳步提升</p>
        ...
        <el-card class="module-card">
          <template #header>
            <div class="card-header">
              <span>词汇量扩展</span>
              <el-tag type="success">基础阶段</el-tag>
            </div>
          </template>
...
const handleVideoClick = (video: any) => {
  ...
  try {
    saveWatchHistory(video, 'teenage7-15')
...
const fetchAllWords = async () => {
  try {
    console.log('Fetching all Teenage 7-15 words for search...')
    const response = await fetch('http://localhost:8080/api/words/age-group/7')
    if (!response.ok) {
      throw new Error('Failed to fetch all Teenage 7-15 words')
    }
    ...
    console.log('All Teenage 7-15 words loaded:', allWords.value.length)
  } catch (error) {
    console.error('Failed to fetch all Teenage 7-15 words:', error)
  }
}
...
const fetchWords = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:8080/api/words/age-group/7/page?page=${currentPage.value - 1}&size=${pageSize.value}`)
...
const fetchVideos = async () => {
  try {
    ...
    const response = await api.videos.getByCategory('青少年英语-7-15岁')
...
const fetchHomework = async () => {
  try {
    ...
    const response = await api.homeworks.getByCategory('青少年英语-7-15岁')
    console.log('API response:', response)
...
<style scoped>
.teenage-english-7-15 {
  padding: 20px 0;
}

.teenage-english-7-15 h2 {
  margin-bottom: 10px;
  color: #303133;
  font-size: 28px;
}
