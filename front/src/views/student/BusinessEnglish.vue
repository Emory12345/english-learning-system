...
const fetchWords = async () => {
  loading.value = true
  try {
    console.log('Fetching Business English words from API...')
    const response = await fetch(`http://localhost:8080/api/words/age-group/30/page?page=${currentPage.value - 1}&size=${pageSize.value}`)
    ...
    console.log('Formatted Business English words:', formattedWords)
    words.value = formattedWords
    totalElements.value = data.totalElements
    totalPages.value = data.totalPages
    
    console.log('Business English words updated:', words.value)
    console.log('Total elements:', totalElements.value)
    console.log('Total pages:', totalPages.value)
  } catch (error) {
    console.error('Failed to fetch words:', error)
    ElMessage.error('获取企业英语单词数据失败，请稍后重试')
  } finally {
    loading.value = false
  }
}
...
const fetchVideos = async () => {
  try {
    ...
    // 过滤并分类企业英语视频
    const businessVideos = response.filter((video: any) => video.category === '企业英语')
    speakingVideos.value = businessVideos.filter((video: any) => video.type === '口语')
    readingVideos.value = businessVideos.filter((video: any) => video.type === '阅读')
    listeningVideos.value = businessVideos.filter((video: any) => video.type === '听力')
    writingVideos.value = businessVideos.filter((video: any) => video.type === '写作')
    console.log('Business English Speaking videos:', speakingVideos.value)
    
    // 检查视频URL
    if (businessVideos.length > 0) {
      console.log('First Business English video URL:', businessVideos[0].videoUrl)
      console.log('Full Business English video URL:', `http://localhost:8080${businessVideos[0].videoUrl}`)
    }
  } catch (error) {
...
const fetchHomework = async () => {
  try {
    ...
    const response = await api.homeworks.getByCategory('企业英语')
    console.log('API response:', response)
