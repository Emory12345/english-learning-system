...
// 四级单词数据
const words = ref([])
const allWords = ref([]) // 存储所有四级单词用于搜索
...
const fetchAllWords = async () => {
  try {
    ...
    console.log('All CET4 words loaded:', allWords.value.length)
  } catch (error) {
...
const fetchWords = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:8080/api/words/age-group/25/page?page=${currentPage.value - 1}&size=${pageSize.value}`)
    ...
    console.log('CET4 words updated:', words.value)
    console.log('CET4 total elements:', totalElements.value)
    console.log('CET4 total pages:', totalPages.value)
  } catch (error) {
